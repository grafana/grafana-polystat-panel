...
import { Button, Card, Field, FieldSet, IconButton, IconName, Input, Select, Switch } from '@grafana/ui';
import { SelectableValue } from '@grafana/data';
import { v4 as UUIDv4 } from 'uuid';

import { CompositeMetricItem } from './CompositeMetricItem';
import { CompositeItemProps, CompositeItemType, CompositeMetric, DisplayModes } from './types';
    ...
    <Card key={`composite-card-${props.ID}`}>
      <Card.Meta>
        <FieldSet>
          ...
          <Field label="Show Members" description="Toggle Display of composite members" disabled={!composite.showComposite}>
            <Switch
              transparent={true}
          ...
          >
            <Select
              ...
              onChange={(v) => {
                setDisplayMode(v);
                setComposite({ ...composite, displayMode: v.value ?? DisplayModes[0].value ?? 'all' });
              }}
